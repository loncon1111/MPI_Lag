...
1100  continue
    ...
    if (myrank .eq. 0) then
        ...
        do irank = 1, nprocs - 1
            ...
            call MPI_IRECV(traint,1,itype(irank),irank,1,MPI_COMM_WORLD,ureq(irank),ierr)

            !call
            !MPI_IRECV(traint(jsta:jend,j,ncol+i),num_rows,itype(irank),irank,1,MPI_COMM_WORLD,ureq(irank),ierr)
        end do
        ...
        call MPI_ISEND(traint,1,itype(myrank),0,1,MPI_COMM_WORLD,ureq1,ierr)

             !call
             !MPI_ISEND(traint(ista:iend,j,ncol+i),num_rows,itype(myrank),0,1,MPI_COMM_WORLD,ureq1,ierr)
