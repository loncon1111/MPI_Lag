...
! ***********************************************************************
    ...
    integer     :: ind10,ind11,ind12,ind13,ind14,ind15,ind16
    integer     :: ind17,ind18,ind19,ind20
    real        :: tmp1,tmp2,tmp3,tmp4
    real        :: absvort,dthdp,dudp,dvdp,dthdx,dthdy ! for calculating Potential Vorticity
    integer     :: noutside
    real        :: delta
    integer     :: itrace0
    character*80:: string
...
11      continue
    ...
    write (delta_x, '(F5.3)') dx
    write (delta_y, '(F5.3)') dy
    delta_x = adjustl(delta_x)
    delta_y = adjustl(delta_y)
    ...
    do while ( i .le. ntrace1 ) 
        ...
        elseif ( tvar(i).eq.'DTDX' ) then
             varname='T'                              ! T
             call add2list(varname,tvar,ntrace1)
             varname='T:+'//trim(delta_x)//'DLON'                            ! T:+'//trim(delta_x)//'DLON
             call add2list(varname,tvar,ntrace1)
             varname='T:-'//trim(delta_x)//'DLON'                            ! T:-'//trim(delta_x)//'DLON
             call add2list(varname,tvar,ntrace1)
        ...
        elseif ( tvar(i).eq.'DTHDY' ) then
             varname='T'                              ! T
             call add2list(varname,tvar,ntrace1)
             varname='T:+'//trim(delta_y)//'DLAT'                            ! T:+'//trim(delta_x)//'DLON
             call add2list(varname,tvar,ntrace1)
             varname='T:-'//trim(delta_y)//'DLAT'                            ! T:-'//trim(delta_x)//'DLON
             call add2list(varname,tvar,ntrace1)
             varname='P'                                   ! P
    ...
    do i = ntrace1, 1, -1
        ...
        trajectory_loop2: do j = ista,iend
             ...
             if ( varsint(i+ncol) .eq. 'TH' ) then
                ...
                print*, 'T', traint(j,k,ind1), 'p', traint(j,k,ind2)               
            ...
            elseif  ( varsint(i+ncol).eq.'DUDX' ) then
                ...
                call calc_DFDX (traint(j,k,ncol+i), traint(j,k,ind1), traint(j,k,ind2),traint(j,k,ind3), dx)
            ...
            elseif  ( varsint(i+ncol).eq.'DTHDX' ) then
                ...
                call calc_DTHDX (traint(j,k,ncol+i), traint(j,k,ind1), traint(j,k,ind2),traint(j,k,ind3),traint(j,k,ind4), dx )
            ...
            elseif  ( varsint(i+ncol).eq.'DUDY' ) then
                ...
                call calc_DFDY (traint(j,k,ncol+i), traint(j,k,ind1), traint(j,k,ind2), dy )
            ...
            elseif ( (varsint(i+ncol).eq.'R_EIGVAL1') .or. (varsint(i+ncol).eq.'R_EIGVAL2') .or.  &
                ...
                varname='U:-'//trim(delta_x)//'DLON'
                call list2ind (ind2,varname,varsint,fok,ncol+ntrace1)
                call calc_DFDY (tmp1, traint(j,k,ind1), traint(j,k,ind2), dy )

                varname='V:+'//trim(delta_x)//'DLON'
                call list2ind (ind1,varname,varsint,fok,ncol+ntrace1)
                varname='V:-'//trim(delta_x)//'DLON'
                call list2ind (ind2,varname,varsint,fok,ncol+ntrace1)
                call calc_DFDY (tmp2, traint(j,k,ind1), traint(j,k,ind2), dy )

                varname='U:+'//trim(delta_y)//'DLAT'
                call list2ind (ind1,varname,varsint,fok,ncol+ntrace1)
                varname='U:-'//trim(delta_y)//'DLAT'
                call list2ind (ind2,varname,varsint,fok,ncol+ntrace1)
                call calc_DFDY (tmp3, traint(j,k,ind1), traint(j,k,ind2), dy )

                varname='V:+'//trim(delta_y)//'DLAT'
                call list2ind (ind1,varname,varsint,fok,ncol+ntrace1)
                varname='V:-'//trim(delta_y)//'DLAT'
                call list2ind (ind2,varname,varsint,fok,ncol+ntrace1)
                call calc_DFDY (tmp4, traint(j,k,ind1), traint(j,k,ind2), dy )
            ...
            elseif  ( varsint(i+ncol).eq.'DTHDY' ) then
                ...
                call calc_DTHDY (traint(j,k,ncol+i), traint(j,k,ind1), traint(j,k,ind2),traint(j,k,ind3), dy )
            ...
            elseif  ( varsint(i+ncol).eq.'RELVORT' ) then
                ...
                call calc_DFDX (tmp2, traint(j,k,ind3),traint(j,k,ind4),traint(j,k,ind5), dx )
                ...
                call calc_RELVORT (traint(j,k,ncol+i), tmp1, tmp2, traint(j,k,ind6),traint(j,k,ind5))
            ...
            elseif  ( varsint(i+ncol).eq.'ABSVORT' ) then
                ...
                call calc_ABSVORT (traint(j,k,ncol+i), tmp1, tmp2, traint(j,k,ind6),traint(j,k,ind5))
            ...
            elseif  ( varsint(i+ncol).eq.'DIV' ) then
                ...
                call calc_DIV (traint(j,k,ncol+i), tmp2, tmp1,traint(j,k,ind6),traint(j,k,ind5))
            ...
            elseif  ( varsint(i+ncol).eq.'DEF' ) then
                ...
                call calc_DFDY (tmp3, traint(j,k,ind6),traint(j,k,ind7), dy )

                varname='U:+'//trim(delta_x)//'DLON'
                call list2ind (ind8,varname,varsint,fok,ncol+ntrace1)
                varname='U:-'//trim(delta_x)//'DLON'
                call list2ind (ind9,varname,varsint,fok,ncol+ntrace1)

                call calc_DFDX(tmp4,traint(j,k,ind8),traint(j,k,ind9),traint(j,k,ind5), dx )
                ...
                call calc_DEF (traint(j,k,ncol+i), tmp4, tmp2,tmp1,tmp3)
            ...
            elseif  ( varsint(i+ncol).eq.'PV' ) then
                ...
                call calc_ABSVORT (absvort, tmp1, tmp2,traint(j,k,ind6),traint(j,k,ind5))
                ...
                call list2ind (ind9,varname,varsint,fok,ncol+ntrace1)
                varname='P:-1DP'
                call list2ind (ind10,varname,varsint,fok,ncol+ntrace1)
                varname='P'
                call list2ind (ind11,varname,varsint,fok,ncol+ntrace1)
                varname='T'
                call list2ind (ind12,varname,varsint,fok,ncol+ntrace1)

                call calc_DTHDP (dthdp, traint(j,k,ind7),traint(j,k,ind8),traint(j,k,ind9),       &
                                traint(j,k,ind10),traint(j,k,ind11),traint(j,k,ind12))

                ! DUDP
                varname='U:+1DP'
                call list2ind (ind13,varname,varsint,fok,ncol+ntrace1)
                varname='U:-1DP'
                call list2ind (ind14,varname,varsint,fok,ncol+ntrace1)

                call calc_DFDP (dudp, traint(j,k,ind13),traint(j,k,ind14),traint(j,k,ind9),traint(j,k,ind10) )
                ...
                call calc_DFDP (dvdp,traint(j,k,ind15),traint(j,k,ind16),traint(j,k,ind9),traint(j,k,ind10) )

                ! DTHDX
                varname='T:+'//trim(delta_x)//'DLON'
                call list2ind (ind17,varname,varsint,fok,ncol+ntrace1)
                varname='T:-'//trim(delta_x)//'DLON'
                call list2ind (ind18,varname,varsint,fok,ncol+ntrace1)
                varname='P'
                call list2ind (ind3,varname,varsint,fok,ncol+ntrace1)
                varname='lat'
                call list2ind (ind4,varname,varsint,fok,ncol+ntrace1)

                call calc_DTHDX (dthdx, traint(j,k,ind17),traint(j,k,ind18),traint(j,k,ind11),traint(j,k,ind5), dx )

                ! DTHDY
                varname='T:+'//trim(delta_y)//'DLAT'
                call list2ind (ind19,varname,varsint,fok,ncol+ntrace1)
                varname='T:-'//trim(delta_y)//'DLAT'
                call list2ind (ind20,varname,varsint,fok,ncol+ntrace1)
                varname='P'
                call list2ind (ind3,varname,varsint,fok,ncol+ntrace1)

                call calc_DTHDY (dthdy, traint(j,k,ind19),traint(j,k,ind20),traint(j,k,ind11), dy )
                ...
                call calc_PV (traint(j,k,ncol+i), absvort, dthdp,dudp,          &
                              dvdp,dthdx,dthdy)
