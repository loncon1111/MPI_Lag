...
! ----------------------------------------------------------------------
    ...
    subroutine calc_THE (the,t,p,q)
        ...
        the = tk*(1000./p)**(0.2854*(1.-0.28*qk)*exp(3.376/(2840.0/(3.5*alog(tk)  &
        &     -alog(100.*p*max(1.E-10,qk)/(0.622+0.378*q))-0.1998)+55.)-0.00254)  &
        &     *1.E3*max(1.E-10,qk)**(1.+0.81*qk))
...
! ----------------------------------------------------------------------
    ...
    subroutine calc_DFDX (dfdx,f1,f0,lat)
        ...
        dfdx = (f1 - f0) / (2. * deltay * cos( lat * pi180 ))
...
! -------------------------------------------------------------------------
    ...
    subroutine calc_DFDY (dfdy,f1,f0)
...
! -------------------------------------------------------------------------
    subroutine calc_DTHDX (dthdx,t1,t0,p,lat)
        ...
        dthdx = (pref/p)**rdcp * (t1-t0) / (2. * deltay * cos(pi180 * lat) )
