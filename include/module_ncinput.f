...
c     Declaration of subroutine parameters 
      ...
      real         dx,dy               ! Horizontal resolution (in degrees)
...
c     Check whether the list of dimensions is OK
      if ( ( dimname(1).ne.'longitude'  ).or.
     >     ( dimname(2).ne.'latitude'  ).or. 
     >     ( dimname(3).ne.'level'  ).and.( dimname(3).ne.'lev_2'  ).or.
     >     ( dimname(4).ne.'time' ) )
     >then
        print*,' ERROR: the dimensions of the variable are not correct'
        print*,'        expected -> longitude / latitude / level / time'
...
c     Check that unit of lev is in hPa - if necessary correct it
      if ( leveltype.eq.'pressure_level' ) then
         varname='level'
         ierr = NF90_INQ_VARID(cdfid,varname,varid)
         IF(ierr /= nf90_NoErr) PRINT *,NF90_STRERROR(ierr)
         ierr = nf90_get_att(cdfid, varid, "units", units)
...
c        print*, times(i)
        if (abs(time-times(i)).lt.eps) then
               isok = 1
               rtime = times(i)
        elseif (timecheck.eqv. .FALSE.) then
               isok = 1
               rtime = times(1)
        endif
      enddo
...
c     Check that surface pressure is in hPa
      ...
      do i=1,vardim(1)
        ...
        enddo
      enddo
c      if ( (maxps.gt.1500.).or.(minps.lt.300.) ) then
c         print*,' ERROR: surface pressre PS must be in hPa'
c         print*,'       ',maxps,minps
c         stop
c      endif
           ...
           do k=1,vardim(3)
             do i=1,vardim(1)
               if ( vertical_swap.eq.1 ) then
                  ...
                  !print*,vertical_swap,i,j,k, p3(i,j,k)
               else
                  p3(i,j,k) = tmp3(i,j,k)
                  !print*,vertical_swap,i,j,k, p3(i,j,k)
               endif
...
c     Read data /or set 3D pressure field) 
      if ( pressure.eq.0 ) then
         ...
         IF(ierr /= nf90_NoErr) PRINT *,NF90_STRERROR(ierr)
         ierr = nf90_get_att(fid, varid, "add_offset", add)
         ierr = nf90_get_att(fid, varid, "scale_factor", factor)
      else
         if (leveltype.eq.'hybrid_sigma_pressure' ) then
            do i=1,vardim(1)
              do j=1,vardim(2)
...
c     Save output array - close array and swap on demand
      do j=1,vardim(2)
        do k=1,vardim(3)
          do i=1,vardim(1)
             if ( vertical_swap.eq.1 ) then
                 field(i,j,k) = tmp3(i,j,vardim(3)-k+1)*factor + add
             else
                 field(i,j,k) = tmp3(i,j,k)*factor + add
             endif
          enddo
