...
startdate="20170713_12"    # Format: YYYYMMDD_HH
enddate="20170715_06"      # Format: YYYYMMDD_HH
interval=1                 # Interval running hours
increment=6                # it is different
hh_total=12		   # Total running hours 
...
startfile='startf_date.4' # Note: 1/2/3/4... prefixes are different file type
calfile='ofile_date.4'	   # 1/2 prefixes are for txt, 4 is for NetCDF file (recommend)
trafile='lsl_date.4'      # You must have 'date' string inside your file output name
...
for ((icase = 0 ; icase < ncases ; icase++));do
    ...
    ./startf.sh ${isdate} ${sfile} "@ ${specifier}" ${stargs} > log.startf.${isdate}   
    if [ -f ./namelist.startf ]; then
       echo " STARTF: Successfully created namelist.startf."
    else
       echo " STARTF: Error in creating namelist.startf. Read 'log.startf.${isdate}' for more details."
       exit 1
    fi
   
    ./startf >> log.startf.${isdate}

    tail -n 3 log.startf.${isdate}
   
    if [ -f ${sfile} ]; then
       echo " STARTF: SUCCESS. Read 'log.startf.${isdate}' for more details before jumping to next step."
    else
       echo " STARTF: Error file is stored in 'log.startf.${isdate}' file"
       exit 2
    fi
