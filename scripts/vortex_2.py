...
#        cs = ax.contour(lons[:],lats[:],mpcalc.smooth_n_point(vort[irow,:,:]*1e4,9,2),clevs,colors='black')
        cs = ax.contour(lons[:],lats[:],vort[irow,:,:]*1e4,clevs,colors='black')
        cs_ow = ax.contour(lons[:],lats[:],mow_lag[irow,:,:],[0],colors='royalblue',linewidth=3)
        cs_ow.monochrome = True
        cs.monochrome = True
