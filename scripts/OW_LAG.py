...
for icase in range(ndate):
    # Forward
    f_fw = netCDF4.Dataset('data/forward/new_%s.nc' %rv_date)
    print(f_fw)
    ...
    # Backward
    f_bw = netCDF4.Dataset('data/backward/new_%s.nc' %rv_date)
    print(f_bw)
...
# fig = plt.figure(figsize=(20,15))
    ...
    for irow,ilev in enumerate(levs):
        ...
        cf = ax.contourf( lons[:],lats[:],ow_lag[irow,:,:]*1e4,
#                              mpcalc.smooth_n_point(vort[irow,:,:]*1e4 ,9,2), 
                                 clevs1,
                         cmap=plt.cm.BrBG_r, extend='both', transform = ccrs.PlateCarree() )
        #cs = ax.contour(lons[:],lats[:],mpcalc.smooth_n_point(ow_lag[irow,:,:]*1e5,9,2),clevs,colors='black')
        
        cs = ax.contour(lons[:],lats[:],ow_lag[irow,:,:],levels=[0],colors='royalblue',linewidths=2.)
        cs.monochrome = True
...
#     axes[irow].set_extent([0, 20, 95, 125], ccrs.PlateCarree())
    ...
    #plt.savefig('owlag_%s.pdf' %rv_date)
    plt.savefig('owlag_%s.png' %rv_date, dpi=300)
