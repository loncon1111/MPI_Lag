...
for icase in range(ndate+1):
    ...
    dst = netCDF4.Dataset('cc3d_%s.nc' %rv_date, "w", format = "NETCDF4" )
    src  = f_fw
    ...
    for ival,val in enumerate(values):
        ...
        in_cc3d = dst.createVariable('labels_cc3d_%05.2f' %val, labels_out.dtype, ('level','latitude','longitude'))
        in_cc3d.long_name = 'Connected Component Labelling %5.2f' %val
        in_cc3d[:,:,:] = labels_out[:,:,:]

        # Flush
        in_cc3d = None; labels_out = None
        reg = None
