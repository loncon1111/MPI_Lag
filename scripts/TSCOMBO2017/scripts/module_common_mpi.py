...
def para_type_block2(imin,imax,jmin,ista,iend,jsta,jend):
    ierr,isize = MPI.Datatype.Get_extent()
    ilen = iend - ista + 1
    jlen = jend - jsta + 1
    itemp = MPI.Datatype.Create_vector(jlen,ilen,imax - imin + 1)
    ...
    inewtype = MPI.Datatype.Creat_struct(2,iblock,idisp)
    inewtype.Commit()
    return inewtype  
