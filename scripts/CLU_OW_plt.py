...
dat_core = pd.read_csv("data/TSCOMBO2017.csv")
    ...
    for ival,val in enumerate(values):
        ...
        if len(no_empty) != 0:
            ...
            for irow,ilev in enumerate(levs):
                ...
                #cs.monochrome = True
                cf = ax.contourf( lons[:],lats[:],labels_out[irow,:,:], transform = ccrs.PlateCarree() )
...
#             axes[irow].set_extent([0, 20, 95, 125], ccrs.PlateCarree())
            ...
            plt.savefig('clu_lag_%05.2f_%s.png' %(val, rv_date), dpi=300)
